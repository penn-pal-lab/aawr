...
class AAWRNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._Q_encoder = h.enc(cfg, modality='state')
        self._V_encoder = h.enc(cfg, modality='state')
        self._pi_encoder = h.enc(cfg, modality='pixels')
...
class AAWR():
    """Implementation of Asymmetric AWR learning + inference."""

    def __init__(self, cfg):
        self.cfg = cfg
        self.device = torch.device(cfg.device)
        self.std = h.linear_schedule(cfg.std_schedule, 0)
        self.model = AAWRNetwork(cfg).to(self.device)
    ...
    def act(self, obs, t0=False, eval_mode=False, step=None):
        ...
        zpi = self.model._pi_encoder(obs['rgb'])
    ...
    def update(self, replay_buffer, step, demo_buffer=None):
        ...
        if self.demo_batch_size > 0:
            ...
            for v in demo_next_obses.values():
                v.squeeze_(0)
        ...
        zq = self.model._Q_encoder(obs['state'])
        zv = self.model._V_encoder(obs['state'])
        zpi = self.model._pi_encoder(obs['rgb'])
        # Compute Q and V learning targets
        with torch.no_grad():
            # next_z = self.model.encode(next_obses)
            next_zv = self.model_target._V_encoder(next_obses['state'])
        ...
        loss = q_value_loss + v_value_loss + pi_loss
        loss.backward()
