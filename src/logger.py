...
CONSOLE_FORMAT = [('episode', 'E', 'int'), ('env_step', 'S', 'int'), ('episode_reward', 'R', 'float'), ('episode_success', 'SC', 'float'), ('total_time', 'T', 'time')]
...
class VideoRecorder:
    ...
    def __init__(self, root_dir, wandb, render_size=384, fps=15):
        ...
        root_dir and make_dir(root_dir / 'eval_video')
        self._wandb = wandb
    ...
    def record(self, env):
        if self.enabled:
            frame = env.render(mode='rgb_array', height=self.render_size, width=self.render_size,camera_id=self.camera_id)
            self.frames.append(frame)

    def save(self, step, prefix=''):
        if self.enabled:
            frames = np.stack(self.frames)
            if self._wandb:
                self._wandb.log({'eval_video': self._wandb.Video(frames.transpose(0, 3, 1, 2), fps=self.fps, format='mp4')}, step=step)
            else:
                imageio.mimwrite(self.save_dir / f'{prefix}{step}.mp4', frames, fps=self.fps)
            ...
            for k, v in d.items():
                if isinstance(v, np.ndarray):
                    if not k.endswith('_hist'):
                        raise ValueError(f'numpy array {k} must have suffix "_hist" to be logged as histogram')
                    v = self._wandb.Histogram(v)
                metrics[f'{category}/{k}'] = v
            self._wandb.log(metrics, step=d['env_step'])
        if category == 'eval':
