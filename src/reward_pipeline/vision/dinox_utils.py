import numpy as np
from PIL import Image
from typing import List, Tuple
import os
import tempfile
import errno
from pathlib import Path

def ensure_dirs(fp):
    """
    Ensure that dirs in the file path exist. 
    Thread-safe creation of directories.

    Args:
        fp (str): File path for which dirs need to be created.
    """
    tmp = os.path.split(fp)[0]
    if not os.path.isdir(tmp):
        try:
            os.makedirs(tmp, exist_ok=True, mode=0o770)
        except OSError as e:
            if e.errno != errno.EEXIST:
                raise
            print(f"multi-thread are creating: {tmp}")


def atomic_write_files(write_function):
    """
    Decorator for writing files atomically. Creates a temporary file and moves it to the final location.

    Args:
        write_function (function): The function to write content to the file.

    Returns:
        function: Wrapped function
    """
    def wrapper(filename, content, save_dir):
        tmp_dir = save_dir

        with tempfile.TemporaryDirectory(dir=tmp_dir) as tmpdirname:
            fp = os.path.join(tmpdirname, filename)
            ensure_dirs(fp)
            write_function(content, fp)
            filepath = os.path.join(save_dir, filename)
            ensure_dirs(filepath)
            os.rename(fp, filepath)  # atomic operation
            os.chmod(filepath, 0o664)
        print(f'save {filepath}')
        return True
    return wrapper




def _decode_and_save_mask(
    mask_rle: dict,
    save_path: Path,
) -> None:
    """Convert RLE -> binary image and save as .npy."""
    counts = _string2rle(mask_rle["counts"])
    mask   = _rle2mask(counts, mask_rle["size"]).astype(np.uint8)  # 0/1
    np.save(save_path, mask)                      # <frame>_<obj>.npy


@staticmethod
def _string2rle(rle_str: str) -> List[int]:
    p = 0
    cnts = []
    while p < len(rle_str) and rle_str[p]:
        x = 0
        k = 0
        more = 1
        while more:
            c = ord(rle_str[p]) - 48
            x |= (c & 0x1f) << 5 * k
            more = c & 0x20
            p += 1
            k += 1
            if not more and (c & 0x10):
                x |= -1 << 5 * k
        if len(cnts) > 2:
            x += cnts[len(cnts) - 2]
        cnts.append(x)
    return cnts

@staticmethod
def _rle2mask(cnts: List[int], size: Tuple[int, int], label=1):
    img = np.zeros(size, dtype=np.uint8)
    ps = 0
    for i in range(0, len(cnts), 2):
        ps += cnts[i]
        for j in range(cnts[i + 1]):
            x = (ps + j) % size[1]
            y = (ps + j) // size[1]
            if y < size[0] and x < size[1]:
                img[y, x] = label
            else:
                break
        ps += cnts[i + 1]
    return img

def _rle2rgba(self, mask_obj) -> Image.Image:
    """
    Convert the compressed RLE string of mask object to png image object.
    :param mask_obj: The :class:`Mask <dds_cloudapi_sdk.tasks.ivp.IVPObjectMask>` object detected by this task
    """
    # convert rle counts to mask array
    rle = self.string2rle(mask_obj.counts)
    mask_array = self.rle2mask(rle, mask_obj.size)
    # convert the array to a 4-channel RGBA image
    mask_alpha = np.where(mask_array == 1, 255, 0).astype(np.uint8)
    mask_rgba = np.stack((255 * np.ones_like(mask_alpha),
                          255 * np.ones_like(mask_alpha),
                          255 * np.ones_like(mask_alpha),
                          mask_alpha),
                         axis=-1)
    image = Image.fromarray(mask_rgba, "RGBA")
    return image