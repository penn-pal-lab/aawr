"""
Process episodes with GSAM object detection.

This module handles running GSAM object detection on episode frames,
...
def _process_episode(
    ...
    """
    GSAM is computed locally on 3 GPU of RL server
    Here we adapt same dinox info format for consistency
    Please refer to ``gsam_detector.py`` for more details
    """
    ...
    def _worker(fidx: int) -> Dict:
        ...
        pred = detector.detect(str(img_path), prompt) 

        frame_info = []
        detector.visualize_bbox_and_mask(pred, vis_dir, f"{fidx:05d}")
        if (len(pred.get("boxes", [])) > 0 and 
            len(pred.get("masks", [])) > 0 and 
            len(pred.get("class_names", [])) > 0):
            for j, (box, mask, class_name, conf) in enumerate(zip(
                pred["boxes"], pred["masks"], pred["class_names"], pred["confidences"]
            )):
                ...
                _write_npy(safe_name, mask, mask_dir)
...
# ───────────────────────── Convenience entry - call this from notebooks ───── #
def test():
    NUM_WORKERS = 2
    DATA_DIR = Path("~/projects/aawr/reward_pipeline/color_seg_config/example_pineapple.jpg")
...
def main():
    ...
    print(f"RUNNING GSAM ON {NUM_WORKERS} WORKERS")
    OBJECT_NAME = sys.argv[3]
    print(f"OBJECT NAME: {OBJECT_NAME}")
    print(f"WARNING: Are you sure to run GSAM on {OBJECT_NAME} in {DATA_ROOT}? Press Enter to continue")
